...
class ChatGPTChatSession:
    ...
    def __init__(
        ...
        Args:
            ...
            model_name (str): 要使用的模型名稱，例如 'gpt-4o', 'gpt-4o-mini', 'gpt-4-turbo', 'gpt-3.5-turbo'
...
使用繁體中文產生小說
        ...
        self.printLog(f"正在初始化 ChatGPT 模型 '{model_name}'...")
        self.printLog("ChatGPT 對話 session 已成功啟動。")
    ...
    def upload_files(self, file_paths: List[str]) -> List[Dict]:
        ...
        supported_text_types = {'.txt', '.md', '.csv', '.json', '.py', '.js', '.html', '.css'}
        ...
        for path in file_paths:
            try:
                ...
                if file_extension in supported_image_types:
                    ...
                    if base64_image:
                        processed_files.append({
                            ...
                            "image_url": {
                                "url": base64_image,
                                "detail": "high"  # 可以是 "low", "high", 或 "auto"
                            }
                        })
                ...
                elif file_extension in supported_text_types:
                    # 處理文字檔案
                    self.printLog(f"正在處理文字檔案：{path}")
                    try:
                        ...
                        processed_files.append({
                            "type": "text",
                            "text": f"檔案名稱: {file_path.name}\n檔案內容:\n{file_content}"
                        })
                        self.printLog(f"✅ 文字檔案處理成功：{path}")
                        ...
                        try:
                            ...
                            self.printLog(f"✅ 文字檔案處理成功（GBK編碼）：{path}")
                        except Exception as e:
                            self.printLog(f"❌ 文字檔案處理失敗：{path}，錯誤：{e}")
                else:
                    self.printLog(f"⚠️  不支援的檔案類型：{path}")
    ...
    def send_message(self, prompt: str, uploaded_files: Optional[List[Dict]] = None):
        """
        將 prompt 和選擇性的檔案傳送給 ChatGPT 模型，並取得回應。
        ...
        Returns:
            str: 來自 ChatGPT 模型的回應文字，或是一則錯誤訊息
        """
        ...
        for attempt in range(max_retries):
            ...
            self.printLog(f"\n正在向 ChatGPT 傳送訊息，{current_datetime}...")
            ...
            try:
                ...
                current_datetime = datetime.now().strftime("%Y/%m/%d %H:%M:%S")
                self.printLog(f"\nChatGPT 已回傳訊息，{current_datetime}...")
                ...
                if is_retryable and attempt < max_retries - 1:
                    self.printLog(
                        f"[{log_time}]呼叫 ChatGPT API 時發生可重試錯誤 (第 {attempt + 1} 次失敗): {e}",
                        True,
                    )
                    # 指數退避邏輯
                    wait_time = min(5 * (2 ** attempt), 60)  # 最大等待 60 秒
                    self.printLog(f"將在 {wait_time} 秒後重試...", True)
                    time.sleep(wait_time)
                else:
                    if attempt >= max_retries - 1:
                        self.printLog(f"[{log_time}]已達到最大重試次數，放棄操作。", True)
                        return f"呼叫 ChatGPT API 失敗，已重試 {max_retries} 次後放棄。最後錯誤：{e}"
                    else:
                        self.printLog(f"[{log_time}]發生不可重試的錯誤: {e}", True)
                        return f"呼叫 ChatGPT API 時發生錯誤：{e}"

    def send_message_stream(self, prompt: str, uploaded_files: Optional[List[Dict]] = None):
        """
        以串流方式發送訊息，實時接收回應。
        
        Args:
            prompt (str): 使用者的文字輸入
            uploaded_files (Optional[List[Dict]]): 使用者上傳的檔案列表
        ...
        try:
            current_datetime = datetime.now().strftime("%Y/%m/%d %H:%M:%S")
            self.printLog(f"\n正在向 ChatGPT 傳送串流訊息，{current_datetime}...")
    ...
    def get_token_usage(self):
        """
        取得最近一次 API 呼叫的 token 使用量資訊。
        注意：這需要在 send_message 方法中儲存 response 物件才能實現。
        """
        # 這個方法需要修改 send_message 來儲存最後的 response
        # 暫時返回提示訊息
        return "Token 使用量資訊需要在 send_message 方法中實現"
    ...
    def set_system_message(self, system_message: str):
        ...
        self.printLog("系統訊息已更新")


# 使用範例
if __name__ == "__main__":
    # 初始化 ChatGPT 對話 session
    # 請替換為你的 OpenAI API 金鑰
    api_key = "your-openai-api-key-here"
    
    # 建立對話 session
    chatgpt_session = ChatGPTChatSession(
        api_key=api_key,
        model_name="gpt-4o-mini",  # 可選: "gpt-4o", "gpt-4-turbo", "gpt-3.5-turbo"
        generation_config={
            "temperature": 0.8,
            "max_tokens": 2000
        }
    ...
    # 發送文字訊息
    response = chatgpt_session.send_message("請寫一個關於未來世界的短篇小說開頭")
    print("ChatGPT 回應:", response)
    
    # 串流方式發送訊息
    print("\n串流回應:")
    for chunk in chatgpt_session.send_message_stream("繼續剛才的故事"):
        print(chunk, end='', flush=True)
    
    # 處理圖片和文字檔案
    # files = chatgpt_session.upload_files(["image.jpg", "document.txt"])
    # response = chatgpt_session.send_message("請分析這些檔案", uploaded_files=files)
    
    # 查看對話歷史
    # print("對話歷史:", chatgpt_session.history)