...
from config import RESPONSE_FILES_DIR, RUN_DIR_PATH_three, TEMP_FOLDER
import base64
import mimetypes
...
@detail_Blueprint.route("/uploadfile", methods=["POST"])
def uploadfile():
    """
...
@detail_Blueprint.route("/doSave", methods=["POST"])
def doSave():
    # 檢查請求是否包含 JSON 資料
    if not request.is_json:
        return jsonify({"error": "請求必須是 JSON 格式"}), 400
    ...
    if dir:
        for item in data["prompts"]:
            if item["type"] == "file" and item["mode"] == "n":
                ...
                src_file_path = os.path.join(TEMP_FOLDER, filename)
...
@detail_Blueprint.route("/runbyid", methods=["POST"])
def runbyid():
